...
knitr::opts_chunk$set(
  ...
  fig.path = 'figures/one-to-n-matches_',
  fig.align = "center", 
  size = "tiny", 
  echo = TRUE, eval = TRUE, 
...
features[["RBD"]] <- c(start = 22517, end = 25381)
...
genomeColors <- rainbow(n = nbGenomes)
names(genomeColors) <- genomeNames
...
```{r match_features, fig.width=10, fig.height=5, out.width="100%", fig.cap="Feature-specific Percent Identical Positions (PIP) profiles. "}
...
for (featureName in names(features)) {
  ...
  featureLength <- 
    featureLimits[["end"]] - featureLimits[["start"]] + 1
